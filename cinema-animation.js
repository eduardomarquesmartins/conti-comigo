...
(() => {
    ...
    class Shard {
        ...
        generateAllTargets() {
            this.targets = [
                Geometries.cloud(), Geometries.megaphone(), Geometries.camera(),
                Geometries.target(), Geometries.drone(), Geometries.logo(logoPoints)
            ];
            this.junk = { x: (Math.random() - 0.5) * W * 3, y: (Math.random() - 0.5) * H * 3, z: (Math.random() - 0.5) * 1500 };
        }
    ...
    function getFloatStep() {
        ...
        if (relS < totalS) {
            ...
            return (stepI === 3) ? 4.0 + sP * 0.7 : (stepI + 1) + (sP * sP) * 0.5;
        }
        const extraS = relS - totalS, logoS = stepH * 0.2;
        return 4.7 + Math.min(1.0, extraS / logoS) * 0.8;
    }
    ...
    function loop() {
        ...
        steps.forEach((step) => {
            ...
            boxes.forEach(box => {
                ...
                // Optimized for 100vh sections: appear sooner, stay longer
                const sR = vH * 1.0, eR = vH * 0.7, fP = Math.max(0, Math.min(1, (sR - rect.top) / (sR - eR)));
                const sFO = vH * 0.3, eFO = vH * 0.0, fOP = Math.max(0, Math.min(1, (rect.top - eFO) / (sFO - eFO)));
                const a = fP * fOP;
                box.style.opacity = a;
                box.style.transform = `translateY(${(1 - a) * 30}px) scale(${0.98 + a * 0.02})`;
