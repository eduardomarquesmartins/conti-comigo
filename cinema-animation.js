...
(() => {
    ...
    const Geometries = {
        ...
        logo: (targetPts) => {
            const p = (targetPts && targetPts.length)
                ? targetPts[Math.floor(Math.random() * targetPts.length)]
                : { x: (Math.random() - 0.5) * 500, y: (Math.random() - 0.5) * 100 };
        ...
        drone: () => {
            ...
            if (r < 0.25) {
                ...
                const t = Math.random();
                if (t < 0.15) return interpolate(-40, -35, -20, -55);
                if (t < 0.35) return interpolate(-20, -55, 20, -55);
                if (t < 0.50) return interpolate(20, -55, 40, -35);
                if (t < 0.75) return interpolate(-110, -35, -40, -35);
                return interpolate(40, -35, 110, -35);
            } else if (r < 0.52) {
                const t = Math.random();
                if (t < 0.4) return interpolate(-80, -35, -80, 50);
                if (t < 0.5) return interpolate(-100, 50, -60, 50);
                if (t < 0.9) return interpolate(80, -35, 80, 50);
                return interpolate(60, 50, 100, 50);
            } else if (r < 0.60) {
                const t = Math.random();
                if (t < 0.35) return interpolate(-110, -35, -145, -60);
                if (t < 0.50) return interpolate(-145, -60, -145, -75);
                if (t < 0.85) return interpolate(110, -35, 145, -60);
                return interpolate(145, -60, 145, -75);
            } else {
    ...
    function sampleTexts() {
        ...
        const tw = 2400, th = 800; off.width = tw; off.height = th;
        ox.fillStyle = "black";
        ox.textAlign = "center";
        ox.textBaseline = "middle";
        ox.font = "900 380px 'Outfit', 'Arial Black', sans-serif";
        ox.clearRect(0, 0, tw, th);
        ox.fillText("& CONTI", tw / 2, th / 2);
        const data = ox.getImageData(0, 0, tw, th).data;
        const pts = [], step = 3;
        for (let y = 0; y < th; y += step) {
            for (let x = 0; x < tw; x += step) {
                if (data[(y * tw + x) * 4 + 3] > 70) {
                    pts.push({ x: (x - tw / 2) * 1.35, y: (y - th / 2) * 1.35 });
                }
            }
        }
        logoPoints = pts.length > 100 ? pts : [];

        ox.clearRect(0, 0, tw, th);
        ox.font = "900 500px 'Outfit', 'Arial Black', sans-serif";
        ox.fillText("I A", tw / 2, th / 2);
        const dataIA = ox.getImageData(0, 0, tw, th).data;
        const ptsIA = [];
        for (let y = 0; y < th; y += step) {
            for (let x = 0; x < tw; x += step) {
                if (dataIA[(y * tw + x) * 4 + 3] > 80) {
                    ptsIA.push({ x: x - tw / 2, y: y - th / 2 });
                }
            }
        }
        iaPoints = ptsIA.length ? ptsIA : [{ x: 0, y: 0 }];
    }
    ...
    class Shard {
        constructor() {
            ...
            if (dice > 0.5) this.color = [15, 23, 42];
            else this.color = [51, 65, 85];
        ...
        generateAllTargets() {
            this.targets = [
                Geometries.cloud(), Geometries.bulb(), Geometries.camera(),
                Geometries.target(),
                Geometries.drone(), Geometries.logo(logoPoints)
            ];
            this.junk = { x: (Math.random() - 0.5) * W * 3, y: (Math.random() - 0.5) * H * 3, z: (Math.random() - 0.5) * 1500 };
        }
        ...
        draw(ctx, fStep, time) {
            ...
            if (T1.isPropeller && T2.isPropeller) {
                ...
                const relX = lx - rx, relZ = lz - (rz || 0), speed = 15.0;
                const dir = (rID === 0 || rID === 2) ? 1 : -1, angle = time * speed * dir;
                const cosA = Math.cos(angle), sinA = Math.sin(angle);
                lx = rx + (relX * cosA - relZ * sinA);
                lz = (rz || 0) + (relX * sinA + relZ * cosA);
            }

            let explosion = 0;
            if (idx >= 0 && idx < 4) { explosion = Math.sin(Math.PI * t) * 0.3; explosion = Math.pow(explosion, 0.5); }
            ...
            else if (idx === 4) cOff = centerOffset.x * (1 - ease);
            else if (idx >= 5) cOff = 0;
            ...
            if (sc > 0 && px > -W && px < W * 2 && py > -H && py < H * 2) {
                ...
                if (T1.isLogo || T2.isLogo) ctx.fillStyle = "#0f172a"; // Visible Dark Navy on White background
                else ctx.fillStyle = `rgb(${this.color[0]},${this.color[1]},${this.color[2]})`;
    ...
    function loop() {
        ...
        steps.forEach((step) => {
            const boxes = step.querySelectorAll('.content-box');
            boxes.forEach(box => {
                const rect = box.getBoundingClientRect(), vH = window.innerHeight;
                const sR = vH * 1.3, eR = vH * 0.9, fP = Math.max(0, Math.min(1, (sR - rect.top) / (sR - eR)));
                const sFO = vH * 0.1, eFO = vH * -0.2, fOP = Math.max(0, Math.min(1, (rect.top - eFO) / (sFO - eFO)));
                const a = fP * fOP;
                box.style.opacity = a;
                box.style.transform = `translateY(${(1 - a) * 60}px)`;
